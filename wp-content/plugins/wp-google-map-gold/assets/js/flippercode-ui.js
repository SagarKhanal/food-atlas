...
jQuery(document).ready(function($) {
    ...
    if ($overviewPage) {
        ...
        //Verify Token
        $('body').on('click','input[name="wpgmp_verify_token"]',function(e){
            e.preventDefault();
            var action_btn = $(this);
            var product_id = $('.fcdoc-product-info').data('product-id');
            var purchase_code = $('input[name="personal_token"]').val();
            if( purchase_code.length <=0 ) {
                 $('input[name="personal_token"]').css('border', '1px solid red');
                return false;
            }
            if (purchase_code && product_id) {
               jQuery.ajax({
                    type: "POST",
                    url: ajaxUrl,
                    dataType: 'json',
                    data: {
                        action: 'verify_purchase_token',
                        product_id: product_id,
                        purchase_code : purchase_code,
                        current_product : $('.fcdoc-product-info').data('current-product-slug'),
                    ...
                    beforeSend: function() {
                        ...
                        $(action_btn).closest('form').find('.fc-msg').hide();
                        //$('input[name="wpgmp_verify_token"]').parent().find('.fcdoc-loader').show();
                    },
                    ...
                    success: function(data) {
						...
						if(data.status == 0 || data.error ) {
							$('input[name="personal_token"]').css('border', '1px solid red');
							update_desc = "<div class='fc-msg fc-danger'>"+data.error+"</div>";  
							$(action_btn).after(update_desc);  
						} else {
							//update_desc = "<div class='fc-msg fc-success'>"+data.success+"</div>";
							$(action_btn).closest('form').find('.token_not_verified').hide();
							$(action_btn).closest('form').find('.token_verified').show();
							$(action_btn).closest('form').find('.token_verified .fc-msg').show();
						}
					}
                });
            } 
        ...
        //Remove Token
        $('body').on('click','input[name="wpgmp_remove_token"]',function(e){
            e.preventDefault();
            var action_btn = $(this);
            var purchase_code = $('input[name="personal_token"]').val();
            if( purchase_code.length <=0 ) {
               ...
               jQuery.ajax({
                    ...
                    success: function(data) {
						...
						if(data.status == 0 || data.error ) {
							...
							update_desc = "<div class='fc-msg fc-success'>"+data.success+"</div>";
							$('input[name="personal_token"]').val('');
							$(action_btn).closest('form').find('.token_not_verified').append(update_desc);
							$(action_btn).closest('form').find('.token_not_verified').show();
							$(action_btn).closest('form').find('.token_verified').hide();
						}
					}
                });
            } 
...
jQuery(document).ready(function($) {
    ...
    $("body").find('.sortable').sortable({placeholder: "ui-sortable-placeholder"});
